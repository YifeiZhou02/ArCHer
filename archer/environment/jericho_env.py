...
class AdventureEnv():
    def __init__(self, rom_path="/global/scratch/users/yifeizhou/20q/jericho_games/z-machine-games-master/jericho-game-suite/detective.z5", max_steps = 60,):
...
class BatchedAdventureEnv():
    def __init__(self, bsize = 32, rom_path="/global/scratch/users/yifeizhou/20q/jericho_games/z-machine-games-master/jericho-game-suite/detective.z5", max_steps = 60):
        #there is a resetting issue, so cannot set bsize too large
        bsize = 4
        self.env_list = [AdventureEnv(rom_path, max_steps) for _ in range(bsize)]
        self.bsize = bsize
        self.reset_obs = None
        self.reset_state = None
